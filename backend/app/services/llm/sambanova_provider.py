...
class SambaNovaProvider(LLMProvider):
    """Provider that calls the SambaNova API (OpenAI-compatible)."""
    ...
    def __init__(self) -> None:
        ...
        self.base_url = settings.SAMBANOVA_BASE_URL.rstrip("/")
        ...
        log.info(
            "[llm:sambanova] initialized — base_url=%s model=%s",
            self.base_url,
            self.model,
        )
        ...
        *,
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout) as client:
                ...
                log.info(
                    "[llm:sambanova] success — model=%s response_time=%.0fms",
                    actual_model,
                    elapsed,
                )
                return LLMResult(
            ...
            elapsed = (time.monotonic() - start) * 1000
            log.warning("[llm:sambanova] failed — model=%s error=%s", self.model, e)
            return LLMResult(
                content="",
                provider=self.name,
                model=self.model,
