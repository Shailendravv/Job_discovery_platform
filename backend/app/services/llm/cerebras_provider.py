...
class CerebrasProvider(LLMProvider):
    """Provider that calls the Cerebras API (OpenAI-compatible)."""
    ...
    def __init__(self) -> None:
        ...
        log.info(
            "[llm:cerebras] initialized — base_url=%s model=%s",
            self.base_url,
            self.model,
        )
        ...
        *,
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout) as client:
                ...
                log.info(
                    "[llm:cerebras] success — model=%s response_time=%.0fms",
                    actual_model,
                    elapsed,
                )
                return LLMResult(
            ...
            elapsed = (time.monotonic() - start) * 1000
            log.warning("[llm:cerebras] failed — model=%s error=%s", self.model, e)
            return LLMResult(
                content="",
                provider=self.name,
                model=self.model,
