"""
Same multi-server agent as Stage 4, but the browser MCP server now
    ...
    async def handle_tools(self, tool_calls) -> dict:
        ...
        if hasattr(msg, "tool_calls") and msg.tool_calls:
            for tc in msg.tool_calls:
                log.debug(
                    "chained tool call: %s(%s)", tc.function.name, tc.function.arguments
                )
...
async def _main():
    ...
    try:
        ...
        log.info(
            "agent session %s; %d tools available across %d servers",
            agent.session_id,
            len(agent.ollama_tools),
            len(agent.mcp_sessions),
        )
