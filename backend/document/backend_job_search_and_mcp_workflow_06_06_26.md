# Backend Job Search & MCP Workflow Reconstruction Guide (June 6, 2026)
...
│   │   ├── search_provider.py     ← updated June 6 (Option A)
│   │   ├── state.py
│   │   ├── nodes/
│   │   │   ├── __init__.py
│   │   │   ├── extract_skills.py
...
### MCP Client & Search Provider (Updated June 6)
...
- MCP `1.27.1` StreamableHTTP transport requires an `initialize` handshake before any `tools/call`. Sending raw JSON-RPC without it returns `400 Bad Request`.
- Previously the `Accept` header was `application/json` only, causing a `406 Not Acceptable`. The SDK now handles headers correctly.
- `call_mcp_tool` is called from a sync context inside FastAPI's running event loop. `asyncio.run()` cannot be called on the same thread, so the async call is dispatched to a `ThreadPoolExecutor` worker which gets its own clean event loop.

#### [app/agents/search_provider.py](file:///d:/AI%20Projects/job_project/project/backend/app/agents/search_provider.py) ← updated (Option A)
```python
"""
...
| | Option B (June 4) | Option A (June 6) |
|---|---|---|
| Call method | Direct Python import of `_do_search()` | HTTP via `call_mcp_tool()` → MCP server |
| MCP server required | No | Yes (`job_mcp_server` on `:8001`) |
| Scalability | Single process only | Independent processes, horizontally scalable |
| Tool used | `search` (text output) | `search_json` (JSON array output) |
...
**Changes from June 4:**
- Added `search_json` tool — returns a JSON array instead of formatted text, consumed by `SearchProvider` (Option A).
- Added `host="0.0.0.0", port=8001` to `FastMCP(...)` so the server binds correctly when run standalone.
- Changed `mcp.run()` → `mcp.run(transport="streamable-http")` to explicitly use the StreamableHTTP transport.
